    ...
    async loadLibraries() {
      try {
        ...
        if (libsData && libsData.length > 0) {
          ...
          console.warn('⚠️ [Bibliotecas] No se encontraron datos en Strapi — lista vacía')
          this.libraries = []
        }
      } catch (error) {
        console.error('❌ [Bibliotecas] Error al cargar desde Strapi:', error.message || error)
        // Si Strapi falla, dejar lista vacía (los datos locales están en BIBLIOTECAS_LOCAL más abajo)
        this.libraries = []
      }
    },
